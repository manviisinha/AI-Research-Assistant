...
from src.assistant.utils import format_documents_with_metadata, invoke_llm, invoke_groq, invoke_ollama, parse_output, tavily_search, Evaluation, Queries
...
def generate_research_queries(state: ResearcherState, config: RunnableConfig):
    ...
    # Using Groq API (fast, free tier available)
    result = invoke_groq(
...
def summarize_query_research(state: QuerySearchState):
    ...
    # Strip any optional <think> reasoning tags
